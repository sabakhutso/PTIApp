...
class PtiRepositoryImpl @Inject constructor(
    ...
    override suspend fun calculateDeadLine(carNumber: String): Result<CalculateDeadLineResult> {
        return try {
            val calculateDeadLine = ptiService.calculateDeadLine(carNumber)
            val responseBody = calculateDeadLine.body()!!
            if (calculateDeadLine.isSuccessful) {
                Result.success(responseBody.toDomain())
            } else {
                Result.failure(IllegalStateException("Response wasn't successful"))
            }
        } catch (e: Exception) {
            Result.failure(e)
